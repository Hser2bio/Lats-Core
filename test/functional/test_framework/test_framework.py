...
class PivxTestFramework():
    ...
    def get_prevouts(self, node_id, utxo_list, nHeight=-1):
        ...
        :return: prevouts:         ({bytes --> (int, bytes, int)} dictionary)
                                   maps CStake "uniqueness" (i.e. serialized COutPoint -or hash stake, for zLATS-)
                                   to (amount, prevScript, timeBlockFrom).
                                   For zLATS prevScript is replaced with serialHash hex string.
        """
    ...
    def stake_block(self, node_id,
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend zLATS inputs when fDoubleSpend is false. It needs to
...
class ComparisonTestFramework(PivxTestFramework):
    ...
    def add_options(self, parser):
        ...
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "latsd"),
                          help="latsd binary to use for reference nodes (if any)")
