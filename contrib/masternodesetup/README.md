...
# LATS Masternode Setup Guide
***
## Required
1) **LATS collateral value at current block** ([consult the collateral table](../../README.md#rewards-breakdown))
2) **Local Wallet https://github.com/LATS-DAO/Lats-Core/releases**
...
Remember to do `lats-cli getblockcount` to check if VPS catching blocks till it synced with chain, if not follow this procedure:

* Go to your wallet-qt and check peers list (tools - peers list) and select one ip from the list. With that ip do the follow command at VPS `lats-cli addnode "ip" onetry`

      Example:
		  lats-cli addnode 45.32.144.158 onetry
    
* Check now if VPS already downloading blocks with the command `lats-cli getblockcount`, and if yes give it time now to catch last block number 
...
```
lats-cli getmasternodestatus
```

You need to get **"status" : 4** 

## Congratulations your LATS node it's running
